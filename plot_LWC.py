...
for line in text:
    ...
    #append 2-item long lists containing time and SNR value to corresponding satellite's list
        ...
        try:
            #The GPS recieves data at a frequency of 1575.42MHz.  We can convert the C/N0 value from the
            #GPS which is in dB-Hz to an SNR value in dB which is used in calculating the SWE.
            #Reciever frequency = 1575.42MHz.  Bandwidth = 10*log(1575.42*1,000,000) = 91.973963545486232243880870991767 dB.
            #SNR = C/N0 - BW = C/N0 - 91.973963545486232243880870991767.  Also C/N0 values are recorded as positive but they are negative.
            sat_SNR.append([int((int(current_time[:2])*3600)+(int(current_time[2:4])*60)+(int(current_time[4:6]))), float((strength_above)-91.973963545486232243880870991767), float(float(check_line[7])-91.973963545486232243880870991767), snow_depth, float(check_line[5])])
...
for i in sat_data:
    ...
    if time_step > (interval + (i[0]-start)) and total_count != len(sat_data):
        ...
        sat_data_ave.append([int(time_ave/count), (sihvola_ave/count), (denoth_ave/count), (roth_ave/count)])
        total_count += 1
...
for x in sat_data_ave:
    time.append(str(int(x[0]/3600)) + ':' + str(int((float(x[0]/3600)-int(x[0]/3600))*60)) + ':' + str(int(int((((float(x[0]/3600) - int(x[0]/3600))*60)-(int((float(x[0]/3600)-int(x[0]/3600))*60)))*60))))
    sihv.append(x[1])
    deno.append(x[2])
    roth.append(x[3])      
...
ax = plt.subplot(111)
ax.set_title('Liquid Water Content (averaged at ' + str(time_step) + ' second intervals)', fontsize=15, fontweight='bold')
